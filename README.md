...
- **Excel — editing fedele all'originale.** (1) **Editor in-cella**: digitando, il testo è ora visibile **dentro la cella** (prima compariva solo nella barra della formula); barra della formula ed editor restano sincronizzati ed editabili in contemporanea. (2) **Copia/incolla con riferimenti relativi**: incollando una formula i riferimenti vengono traslati (`=A1*2` → `=A2*2`), conservando quelli assoluti (`$A$1`). (3) **"Punta e clicca"**: durante una formula si può cliccare/trascinare per inserire celle e intervalli (es. `=SOMMA(A1:A10)`). (4) **Evidenziazione dei riferimenti**: modificando una cella con formula, i range usati sono evidenziati con **riquadri colorati**. (5) **Bordi**: nuovo menu **visuale** (campioni di linea, colore, anteprime live) con spessore e tipo (continua, tratteggiata, doppia…), gamma completa di posizioni **range-aware** (esterni vs interni orizzontali/verticali) e rimozione bordi ora funzionante. Coperto dal nuovo test `clone-tests/07-excel-editing.mjs`.
- **Funzioni server in modalità demo (host statico)** — su GitHub Pages / `file://` il backend PHP non gira: i comandi *Salva/Apri dal server* tentavano comunque la chiamata e fallivano con un errore di parsing (`Unexpected token '<'`, l'app riceveva HTML invece di JSON). Ora una **guardia demo** (helper `cloneOfficeDemoNotice()` in `js/auth.js`) mostra un **avviso** e blocca la chiamata, invitando a usare il salvataggio locale. Nessun effetto in locale o su un host con PHP (es. Altervista).
- **Access su mobile — modale "Apri dal server" spostata/tagliata a destra** — la barra superiore andava in *overflow* orizzontale (testo intero "Acquista Clone Office") allargando la pagina oltre il viewport, così l'overlay `position:fixed` si centrava su una larghezza maggiore del visibile. Risolto con `overflow:hidden`/`min-width:0` sulla barra e versione **compatta su mobile** ("Acquista" solo icona); la modale è ora centrata.
